...
server <- function(input, output, session) {
  ...
  observeEvent(input$reset, {
    ...
    updateSelectInput(session, 'maintopic', selected = character(0))
  ...
  observe({
    ...
    if (length(input$maintopic)==0 || length(input$dept)==0) {
      ...
      leafletProxy('heatmap') %>% clearGroup('heat')
      # more plots
    } else if (input$maintopic == 'all' & input$dept == 'all') {
      ...
      circle_data = sm1 %>% filter(Assigned.Department %in% input$dept & TopicBig %in% input$maintopic & Request.Date>input$dates[1] & Request.Date<input$dates[2])
      heat_data = sm1 %>% filter(Assigned.Department %in% input$dept & TopicBig %in% input$maintopic & Request.Date>input$dates[1] & Request.Date<input$dates[2])
    }
    ...
    leafletProxy("circlemap", data = circle_data) %>%
      ...
      addCircles(~Longitude, ~Latitude, radius=20, layerId=~Request.ID,
                 stroke=FALSE, fillOpacity=0.6, fillColor='red', group='circles')
      # addLegend("bottomleft", pal=pal, values=colorData, title='Departments', layerId="colorLegend") 
    
    ## add the heatmap
    leafletProxy("heatmap", data = heat_data) %>%
      clearGroup('heat') %>%
      addWebGLHeatmap(lng=~Longitude,lat=~Latitude,size=20,units='px',alphaRange=0.2,group='heat')
    ...
    output$dept_analysis <- renderPlot({
      ...
      #   return(NULL)
      dept_analysis(sm_full,input$maintopic,input$dates[1],input$dates[2])
    })
    ...
    output$time_analysis <- renderPlot({
      ...
      # if (nrow(requestsInBounds()) == 0)
      #   return(NULL)
      time_analysis(sm_full,input$maintopic,input$dates[1],input$dates[2])
    })
